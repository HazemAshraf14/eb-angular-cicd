...
export class BarometerGraphComponent implements OnInit {

  public options;

  @Input('barometerData') barometerData;
  ...
  ngOnInit(): void {
    ...
    let chart = Highcharts.stockChart('barometerContainer', this.options);
    ...
    let drawData = this.tagsInfoService.getterBarometerData();
  ...
  exportCSV(){
    ...
    let exportbtnElement = document.getElementById('exportBarometerBtn') as HTMLInputElement | null;
    exportbtnElement.disabled = true;
    setTimeout(async ()=>{
    let isDownloaded = await this.excelService.generateExcel(this.tagsInfoService.getterBarometerData(), 'Barometer');
