...
export class ZAccelerometerGraphComponent implements OnInit {

  public options;

  @Input('zAccelerometerData') zAccelerometerData;
  ...
  ngOnInit(): void {
    ...
    let chart = Highcharts.stockChart('zAccelerometerContainer', this.options);
    ...
    let drawData = this.tagsInfoService.getterZAccelerometerData();
  ...
  exportCSV(){
    ...
    let exportbtnElement = document.getElementById('exportZAccelerometerBtn') as HTMLInputElement | null;
    exportbtnElement.disabled = true;
    setTimeout(async ()=>{
    let isDownloaded = await this.excelService.generateExcel(this.tagsInfoService.getterZAccelerometerData(), 'Accelerometer-Z');
